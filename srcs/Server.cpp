...
void Server::servInit() {
    ...
    data_sock.sin_addr = INADDR_ANY; // Prends toutes les addresses locales 
    ...
    if (fcntl(_serverSocketFd, F_SETTL, O_NONBLOCK) == -1) // Le socket ne block plus lors d'une tache 
    {
        close(_serverSocketFd);
        throw(std::exception);
    }
